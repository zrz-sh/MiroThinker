...
def generate_final_answer(task_description: str, message_history: list, client: OpenAI, model: str) -> str:
    ...
    prompt = f"""You are a data extraction assistant. Your ONLY job is to output a markdown table answer based on the task and collected data.
...
## Output Requirements:
1. Output ONLY a markdown table answer wrapped in ```markdown and ```
2. Use STANDARD MARKDOWN TABLE FORMAT with pipe characters (|), like this:
   | Column1 | Column2 | Column3 |
   |---------|---------|---------|
   | data1   | data2   | data3   |
3. Include ALL rows that have ANY data - do NOT skip rows even if some columns are missing
4. For any missing cell data, use "N/A" instead of omitting the entire row
5. Start your response directly with ```markdown

Output:"""

    try:
        system_msg = "You are a data extraction assistant. Output ONLY markdown tables. Never use <think> tags. Never call tools. Never explain."
...
def main():
    ...
    for run_num in run_numbers:
        run_folder = os.path.join(args.log_dir, f'run_{run_num}')
        output_folder = os.path.join(args.log_dir, f'run_{run_num}_append_final_box_answer')
