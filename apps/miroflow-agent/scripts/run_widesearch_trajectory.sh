...
NUM_RUNS=${NUM_RUNS:-1}                    # Number of parallel runs (each run processes all tasks)
NUM_TASKS=${NUM_TASKS:-null}                  # Number of tasks to process (use first N tasks from dataset, null for all)
MAX_CONCURRENT=${MAX_CONCURRENT:-3}        # Max concurrent tasks within each run (3-5 for 2xA100 80GB)
